        ...
        for entry in customer_sections:
            ...
            cur.execute("""
                ...
                WHERE cust_id = %s AND fin_year = %s
                  AND (UPPER(TRIM(tds_section)) = %s OR tds_section = '—' OR tds_section IS NULL)
...
def get_db():
    try:
        return mysql.connector.connect(
            ...
            password  = os.getenv('DB_PASSWORD', '@Pooky2026'),
...
def upload_books_data():
    ...
    try:
        ...
        conn.commit()
        _close(cur, conn)
        # Recalc TDS deductible, then sync to BigQuery after successful Books upload
        update_tds_deductible_from_section(fin_year)
...
@app.route('/api/recalc-tds/<fin_year>', methods=['POST'])
def recalc_tds(fin_year):
    """Manually trigger tds_deductible recalculation for a given FY.
    POST /api/recalc-tds/2023-24
    """
    try:
        update_tds_deductible_from_section(fin_year)
        return jsonify({'status': 'success', 'message': f'TDS deductible recalculated for FY {fin_year}'}), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 500
