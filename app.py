...
def update_tds_deductible_from_section(fin_year):
    ...
    try:
        ...
        for entry in customer_sections:
            ...
            # Step 4 — Fetch individual lines to apply contextual calculations
            cur.execute("""
                SELECT income_id, taxable_amt, tax_amt
                FROM income_records
                WHERE cust_id = %s AND fin_year = %s
            """, (cust_id, fin_year))
    ...
    try:
        ...
        cur.execute("""
            SELECT
                ...
                CASE WHEN COALESCE(SUM(ir.tax_amt), 0) = 0 THEN 0.00
...
def books_summary():
    ...
    SELECT
        ...
        COALESCE(SUM(i.tax_amt), 0.00) AS tds_deductible,
        COALESCE(SUM(GREATEST(i.gross_amt - i.tax_amt, 0)), 0.00) AS collection_amt,
        COALESCE(SUM(i.tax_amt), 0.00) AS tds_receivable
    FROM cust_master c
...
def get_books_detailed_real():
    ...
    SELECT 
        ...
        tax_amt AS tds_deductible, 
        
        (gross_amt - tax_amt) AS collection_amt, 
        tax_amt AS tds_receivable, 
...
def upload_books_data():
    ...
    try:
        ...
        # Load TDS rates keyed by cust_id
        cur.execute('SELECT cust_id, tds_section, tds_rate FROM tds_codes_master ORDER BY eff_from_date DESC')
        tds_by_cust = {}
        for t in cur.fetchall():
            if t['cust_id'] not in tds_by_cust:
                tds_by_cust[t['cust_id']] = {'tds_section': t['tds_section'], 'tds_rate': float(t['tds_rate'])}
        ...
        for i, row in enumerate(rows_data, start=2):
            ...
            doc_type = (_get(row, 'document_type','doc_type','type') or 'invoice').lower()
            ...
            # ════════════════════════════════════════════════════════
            # First, check if the uploaded Excel row contains a section header column
            excel_section = _get(row, 'tds_section', 'section_code', 'section', 'sec')
            
            # If the excel cell is empty, fallback dynamically to the TDS Codes Master definition
            if not excel_section or excel_section in ('—', '-', 'None', ''):
                tds_info = tds_by_cust.get(cust_id, {'tds_section': '—', 'tds_rate': 0.0})
                tds_section = tds_info['tds_section'] or '—'
                tds_rate = tds_info['tds_rate']
            else:
                # If section exists in Excel, preserve it and resolve rate from master fallback
                tds_section = excel_section
                tds_info = tds_by_cust.get(cust_id, {'tds_section': tds_section, 'tds_rate': 0.0})
                tds_rate = tds_info['tds_rate']
            ...
            if ext == 'txt':
                ...
                entry_source = 'sales_register' if is_sales_register else 'manual_entry'
...
def get_recon_stats():
    ...
    try:
        ...
        cursor.execute("""
            ...
            FROM cust_master c
            LEFT JOIN ( 
                SELECT cust_id, SUM(tax_amt) AS system_tds, SUM(gross_amt) AS gross_income 
                FROM income_records 
                WHERE fin_year = %s
                GROUP BY cust_id
            ...
            FROM (
                SELECT 
                    COALESCE(ro.manual_status, CASE 
                        ...
                        WHEN ABS(COALESCE(g.f26as_credit, 0.00) - COALESCE(b.system_tds, 0.00)) < 1.00 THEN 'reconciled'
            ...
            SELECT * FROM (
                SELECT 
                    ...
                    CASE 
                        ...
                        WHEN ABS(COALESCE(b.system_tds, 0.00) - COALESCE(g.f26as_credit, 0.00)) < 1.00 THEN 100.00
        ...
        query = """
            ...
            LEFT JOIN (
                SELECT cust_id,
                       ...
                       SUM(tax_amt) AS total_deductible,
                       COUNT(*) AS inv_count
                FROM income_records 
                WHERE fin_year = %s
                GROUP BY cust_id
